...
def open_not_empty(open_set):
    return bool(open_set[0])  # Return True if the heap is not empty


def get_best(open_set):
    # Get the best node (lowest heuristic cost)
    key = heapq.heappop(open_set[0])[1]
...
def duplicate_in_open(vn, open_set):
    if vn in open_set[1]:
        ...
        if vn.g < existing_cost:
            # Update the dictionary and heap with the new cost
            open_set[1][vn] = vn.g
            # Remove old entry from the heap
            open_set[0] = [(h, n) for h, n in open_set[0] if n != vn]
            # Reinsert the updated node into the heap
            heapq.heappush(open_set[0], (vn.f, vn))
            heapq.heapify(open_set[0])  # Re-heapify after modification
            return False  # Not a duplicate, updated with better cost
        return True  # Duplicate with an equal or worse cost
    ...
    for index, state in enumerate(closed_set):
        if state.state == vn.state:
            if vn.g < state.g:
                ...
                closed_set[index] = vn
                return False  # Not a duplicate, replaced with better cost
            return True  # Duplicate with an equal or worse cost
    return False  # Not a duplicate
